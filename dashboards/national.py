...
def render_maternal_dashboard_shared(
    ...
    logging.info(
        f"📅 NATIONAL: enrollment_date - {valid_dates}/{total_patients} valid dates"
    )
...
def render():
        ...
        if st.session_state.filter_mode == "All Facilities":
            ...
            for fac_name in st.session_state.selected_facilities:
                for region_name, facilities in facilities_by_region.items():
                    ...
                    if fac_name in facility_names:
                        selected_by_region[region_name] = (
                            selected_by_region.get(region_name, 0) + 1
                        )
                        break
            ...
            if len(selected_by_region) == 0:
                ...
                for region_name, count in selected_by_region.items():
                    ...
                    region_texts.append(
                        f"{count}/{total_in_region} in {short_region_name}"
                    )
                display_text = f"Selected: {', '.join(region_texts)}"
            else:
                total_selected = len(st.session_state.selected_facilities)
                display_text = (
                    f"Selected: {total_selected}/{total_facilities} in "
                    f"{len(selected_by_region)} regions"
                )
        else:
            display_text = f"Selected: All ({total_facilities})"
    ...
    st.markdown(
        ...
        .st-key-national_active_tab_selector [data-testid="stRadio"] div[role="radiogroup"][aria-orientation="horizontal"] {
            gap: 0.45rem;
            margin-top: 0 !important;
        }
        .st-key-national_active_tab_selector [data-testid="stRadio"] label {
            margin: 0 !important;
        ...
        .st-key-national_active_tab_selector [data-testid="stRadio"] label:has(input:checked) {
            border-color: #1d4ed8;
    ...
    if selected_tab == "maternal":
        ...
        render_hfa_tab(user, key_prefix="hfa_national")

    elif selected_tab == "tracking":
        # National user tracks Regional and Facility users
        render_usage_tracking_shared('national')
