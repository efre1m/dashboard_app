...
def render():
    ...
    # Compute KPIs for the selected regions
    kpis = compute_kpis(copied_events_df, region_uids)
    if not isinstance(kpis, dict):
        st.error("Error computing KPI. Please check data.")
        return

    # Calculate trends for the cards
    ippcar_trend, ippcar_trend_class = calculate_trend(copied_events_df, "ippcar", region_uids)
    stillbirth_trend, stillbirth_trend_class = calculate_trend(copied_events_df, "stillbirth", region_uids)
    pnc_trend, pnc_trend_class = calculate_trend(copied_events_df, "pnc", region_uids)
    maternal_death_trend, maternal_death_trend_class = calculate_trend(copied_events_df, "maternal_death", region_uids)
    csection_trend, csection_trend_class = calculate_trend(copied_events_df, "csection", region_uids)
