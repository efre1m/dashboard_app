...
def render():
    ...
    with st.spinner("Fetching maternal data..."):
        try:
            ...
            st.error("⚠️ Connection problem: DHIS2 data could not be fetched within 3 minutes.")
            return
        except requests.RequestException as e:
            st.error(f"⚠️ DHIS2 request failed: {e}")
            return
        except Exception as e:
            st.error(f"⚠️ Unexpected error while fetching data: {e}")
            return
    ...
    with col2:
        ...
        period_label = st.selectbox("⏰ Aggregation Level", ["Daily", "Monthly", "Quarterly", "Annual"])
    ...
    # ---------------- Apply Time Filters ----------------
    if not enrollments_df.empty and "enrollmentDate" in enrollments_df.columns:
        enrollments_df = enrollments_df[(enrollments_df["enrollmentDate"].dt.date >= start_date) &
                                        (enrollments_df["enrollmentDate"].dt.date <= end_date)]
        enrollments_df = assign_period(enrollments_df, "enrollmentDate", period_label)
    if not delivery_df.empty:
        delivery_df = delivery_df[(delivery_df["event_date"].dt.date >= start_date) &
    ...
    with col1:
        ...
        if kpi_selection=="Maternal Admissions":
            ...
            trend_group = filtered_df.groupby("period")["tei_id"].nunique().reset_index(name="value") if not filtered_df.empty else pd.DataFrame()
            render_trend_chart(trend_group,"period","value",f"Maternal Admissions Trend ({status_filter})",bg_color,text_color,"line")
        elif kpi_selection=="Instrumental Delivery Rate":
            if not delivery_df.empty:
                idr_group = delivery_df.groupby("period").apply(lambda x: pd.Series({"value": x[(x.get("dataElementName")=="Instrumental Delivery") & (x["value"].astype(str).str.lower()=="true")]["tei_id"].nunique()/max(1,x[x.get("dataElementName")=="Total Deliveries"]["tei_id"].nunique())*100})).reset_index()
            else: idr_group=pd.DataFrame()
            render_trend_chart(idr_group,"period","value",f"Instrumental Delivery Rate (%)",bg_color,text_color,"bar")
        elif kpi_selection=="Cesarean Section Rate":
            if not delivery_df.empty:
                csr_group = delivery_df.groupby("period").apply(lambda x: pd.Series({"value": x[(x.get("dataElementName")=="Delivery Type") & (x["value"]=="C-Section")]["tei_id"].nunique()/max(1,x[x.get("dataElementName")=="Delivery Type"]["tei_id"].nunique())*100})).reset_index()
            else: csr_group=pd.DataFrame()
            render_trend_chart(csr_group,"period","value",f"Cesarean Section Rate (%)",bg_color,text_color,"bar")
            ...
            if not delivery_df.empty:
                md_group = delivery_df[(delivery_df.get("dataElement_uid")=="TjQOcW6tm8k") & (delivery_df.get("value")=="4")].groupby("period")["tei_id"].nunique().reset_index(name="value")
            else: md_group=pd.DataFrame()
            if md_group.empty:
                md_group=pd.DataFrame({"period":[f"{start_date} to {end_date}"],"value":[0]})
            render_trend_chart(md_group,"period","value","Maternal Deaths Trend",bg_color,text_color,"line")
        st.markdown('</div>', unsafe_allow_html=True)
