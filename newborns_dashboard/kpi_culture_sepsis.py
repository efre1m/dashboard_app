...
from newborns_dashboard.kpi_culture_done import compute_culture_done_numerator
...
def compute_sepsis_culture_numerator(df, facility_uids=None):
    """
    Compute numerator for Culture Done for Sepsis KPI:
    Count of babies with Probable Sepsis who had blood culture done

    Reuses the existing compute_culture_done_numerator function
    """
    return compute_culture_done_numerator(df, facility_uids)


def compute_sepsis_cases(df, facility_uids=None):
    """
    Compute denominator for Culture Done for Sepsis KPI:
    Total count of babies with SubCategoriesofInfection ("Probable Sepsis")
    """
    ...
    # Count babies with Probable Sepsis
    sepsis_mask = (df["dataElement_uid"] == SUBCATEGORIES_INFECTION_UID) & (
        df["value"] == PROBABLE_SEPSIS_CODE
    )

    sepsis_events = df[sepsis_mask]
    sepsis_cases = sepsis_events["tei_id"].nunique()

    return sepsis_cases


def compute_culture_done_sepsis_kpi(df, facility_uids=None, tei_df=None):
    """
    ...
    Formula: % Culture done for babies with clinical sepsis =
             ...
             (Total babies with SubCategoriesofInfection ("Probable Sepsis")) × 100
    """
...
def compute_culture_done_sepsis_trend_data(
    df, period_col="period_display", facility_uids=None, tei_df=None
):
    """
    Compute Culture Done for Sepsis trend data by period

    Returns:
        DataFrame with columns: period_display, sepsis_count, culture_count, culture_sepsis_rate
    """
    if df is None or df.empty:
        return pd.DataFrame()
    ...
    for period in periods:
        ...
        period_kpi = compute_culture_done_sepsis_kpi(period_df, facility_uids, tei_df)

        trend_data.append(
            {
                period_col: period,
                "sepsis_count": period_kpi["sepsis_count"],
                "culture_count": period_kpi["culture_count"],
                "culture_sepsis_rate": period_kpi["culture_sepsis_rate"],
            }
        )
...
# ---------------- CHART FUNCTIONS ----------------
def render_culture_done_sepsis_trend_chart(
    df,
    ...
    # Compute trend data
    trend_df = compute_culture_done_sepsis_trend_data(
    ...
    fig.update_traces(
        line=dict(
            width=3, color="#ff7f0e"
        ),  # Different color from regular culture done
        marker=dict(size=8, color="#ff7f0e"),
        hovertemplate="<b>%{x}</b><br>Culture Rate: %{y:.1f}%<br>Culture Done: %{customdata[0]}/%{customdata[1]}<extra></extra>",
        customdata=trend_df[["culture_count", "sepsis_count"]].values,
    )
    ...
    fig.update_layout(
        ...
        xaxis_title="Period",
        yaxis_title="Culture Done for Sepsis Cases (%)",
        xaxis=dict(
            type="category",
            tickangle=-45,
            showgrid=True,
        ...
        yaxis=dict(
            range=[0, 105] if trend_df["culture_sepsis_rate"].max() > 0 else [0, 100],
            showgrid=True,
            gridcolor="rgba(128,128,128,0.2)",
            zeroline=True,
    ...
    if len(trend_df) > 1:
        last_value = trend_df["culture_sepsis_rate"].iloc[-1]
        prev_value = trend_df["culture_sepsis_rate"].iloc[-2]
        trend_symbol = (
            "▲"
    ...
    # Show summary table
    st.subheader("📋 Culture Done for Sepsis Cases Trend by Period")

    if not trend_df.empty:
        # Calculate overall totals
        overall_data = compute_culture_done_sepsis_kpi(df, facility_uids, tei_df)
        ...
        table_data = table_data.rename(
            columns={
                ...
                "sepsis_count": "Total Probable Sepsis Cases",  # SECOND
                "culture_sepsis_rate": "Culture Rate for Sepsis (%)",
            }
        )

        # Create Overall row with CORRECT COLUMN ORDER
        overall_row = {
            "Period": "Overall",
            "Culture Done": overall_data["culture_count"],  # FIRST
            "Total Probable Sepsis Cases": overall_data["sepsis_count"],  # SECOND
            "Culture Rate for Sepsis (%)": overall_data["culture_sepsis_rate"],
        }
        ...
        # Ensure CORRECT COLUMN ORDER in the final table
        column_order = [
            "Period",
            "Culture Done",
            "Total Probable Sepsis Cases",
            "Culture Rate for Sepsis (%)",
        ]
        ...
        styled_table = (
            table_data.style.format(
                {
                    "Culture Done": "{:,.0f}",
                    "Total Probable Sepsis Cases": "{:,.0f}",
                    "Culture Rate for Sepsis (%)": "{:.1f}%",
                }
            )
        ...
        st.download_button(
            label="Download CSV",
            data=csv,
            file_name="culture_done_sepsis_trend.csv",
            mime="text/csv",
        )
    else:
...
def render_culture_done_sepsis_facility_comparison_chart(
    df,
    period_col="period_display",
    ...
    """
    Render facility comparison chart for Culture Done for Sepsis KPI
    """
    if text_color is None:
        text_color = auto_text_color(bg_color)
    ...
    for period_display in all_periods:
        ...
        for facility_uid, facility_name in facility_uid_to_name.items():
            ...
            if not facility_period_df.empty:
                facility_kpi = compute_culture_done_sepsis_kpi(
                ...
                time_series_data.append(
                    {
                        ...
                        "value": facility_kpi["culture_sepsis_rate"],
                        "culture_count": facility_kpi["culture_count"],
                        "sepsis_count": facility_kpi["sepsis_count"],
                    }
                )
    ...
    fig.update_traces(
        ...
        customdata=time_series_df[["culture_count", "sepsis_count"]].values,
    )
    ...
    for facility_uid, facility_name in facility_uid_to_name.items():
        ...
        if not facility_df.empty:
            culture_data = compute_culture_done_sepsis_kpi(
            ...
            table_data.append(
                {
                    ...
                    # CORRECT ORDER: Culture Done first, then Sepsis Cases
                    "Culture Done": culture_data["culture_count"],
                    "Probable Sepsis Cases": culture_data["sepsis_count"],
                    "Culture Rate for Sepsis (%)": culture_data["culture_sepsis_rate"],
                }
            )
    ...
    table_df = table_df.sort_values("Culture Rate for Sepsis (%)", ascending=False)

    # Calculate overall totals
    overall_data = compute_culture_done_sepsis_kpi(filtered_df, facility_uids, tei_df)

    # Create Overall row with CORRECT ORDER
    overall_row = {
        "Facility Name": "Overall",
        "Culture Done": overall_data["culture_count"],  # FIRST
        "Probable Sepsis Cases": overall_data["sepsis_count"],  # SECOND
        "Culture Rate for Sepsis (%)": overall_data["culture_sepsis_rate"],
    }
    ...
    st.download_button(
        label="Download CSV",
        data=csv,
        file_name="culture_done_sepsis_facility_comparison.csv",
        mime="text/csv",
    )


def render_culture_done_sepsis_region_comparison_chart(
    df,
    period_col="period_display",
    ...
    """
    Render region comparison chart for Culture Done for Sepsis KPI
    """
    if text_color is None:
        text_color = auto_text_color(bg_color)
        ...
        for region_name in region_names:
            ...
            if region_facility_uids:
                ...
                if not region_period_df.empty:
                    region_kpi = compute_culture_done_sepsis_kpi(
                    ...
                    time_series_data.append(
                        {
                            "period_display": period_display,
                            "Region": region_name,
                            "value": region_kpi["culture_sepsis_rate"],
                            "culture_count": region_kpi["culture_count"],
                            "sepsis_count": region_kpi["sepsis_count"],
                        }
                    )
                ...
                table_data.append(
                    {
                        ...
                        "Culture Done": culture_data["culture_count"],
                        "Probable Sepsis Cases": culture_data["sepsis_count"],
                        "Culture Rate for Sepsis (%)": culture_data[
                            "culture_sepsis_rate"
                        ],
                    }
                )
    ...
    overall_data = compute_culture_done_sepsis_kpi(
    ...
    st.download_button(
        label="Download CSV",
        data=csv,
        file_name="culture_done_sepsis_region_comparison.csv",
        mime="text/csv",
    )


# ---------------- COMPREHENSIVE SUMMARY ----------------
def render_culture_done_sepsis_comprehensive_summary(
    df,
    ...
    # Compute KPI
    culture_data = compute_culture_done_sepsis_kpi(df, facility_uids, tei_df)
    ...
    with col3:
        # CORRECT ORDER: Probable Sepsis Cases second
        st.metric(
    ...
    # Add comparison with antibiotics KPI if available
    try:
        from newborns_dashboard.kpi_antibiotic import compute_antibiotics_kpi

        st.subheader("📊 Comparison with Antibiotics Use")

        antibiotics_data = compute_antibiotics_kpi(df, facility_uids, tei_df)

        comparison_data = {
            "Metric": [
                "Culture Done for Sepsis Rate",
                "Antibiotics for Sepsis Rate",
                "Gap Analysis",
            ],
            "Value": [
                f"{culture_data['culture_sepsis_rate']:.1f}%",
                f"{antibiotics_data['antibiotics_rate']:.1f}%",
                f"{antibiotics_data['antibiotics_rate'] - culture_data['culture_sepsis_rate']:.1f}%",
            ],
            "Description": [
                "Culture done rate for sepsis cases",
                "Antibiotics administration rate for sepsis cases",
                "Difference between antibiotics use and culture testing",
            ],
        }

        comparison_df = pd.DataFrame(comparison_data)

        styled_comparison = comparison_df.style.set_table_attributes(
            'class="summary-table"'
        ).hide(axis="index")

        st.markdown(styled_comparison.to_html(), unsafe_allow_html=True)
    except ImportError:
        # If antibiotics KPI not available, skip comparison
        pass
...
__all__ = [
    ...
    "compute_culture_done_sepsis_trend_data",
