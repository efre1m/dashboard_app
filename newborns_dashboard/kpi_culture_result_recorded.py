# kpi_culture_result_recorded.py
import pandas as pd
...
# ---------------- KPI Constants ----------------
# Result codes (same as in kpi_culture_done.py)
CULTURE_RESULT_RECORDED_VALUES = [
    "1",
    "2",
]  # 1=Negative, 2=Positive (EXCLUDES 3=Unknown)
# CULTURE_DONE_VALUES = ["1", "2", "3"]  # Includes all results (from imported)


# ---------------- KPI Computation Functions ----------------
def compute_culture_result_recorded_numerator(df, facility_uids=None):
    """
    Compute numerator for Culture Result Recorded KPI:
    Count of babies with blood culture done AND result recorded (Negative or Positive)

    Formula: Count where:
        - Blood culture for suspected sepsis = "1" or "2"
          (1=Done - Culture Negative, 2=Done - Culture Positive)
          EXCLUDES "3" (Done but Unknown Result)
    """
    ...
    culture_result_cases = df[
        (df["dataElement_uid"] == BLOOD_CULTURE_UID)
        & df["value"].notna()
        & (df["value"].isin(CULTURE_RESULT_RECORDED_VALUES))
    ]["tei_id"].nunique()

    return culture_result_cases


def compute_culture_result_recorded_denominator(df, facility_uids=None):
    """
    Compute denominator for Culture Result Recorded KPI:
    Total count of babies with blood culture done (any result)

    Reuses the existing compute_culture_done_numerator function which counts
    all culture done cases (Negative, Positive, and Unknown)
    """
    return compute_culture_done_numerator(df, facility_uids)


def compute_culture_result_recorded_kpi(df, facility_uids=None, tei_df=None):
    """
    ...
    culture_result_count = compute_culture_result_recorded_numerator(df, facility_uids)

    # Count all babies with culture done (denominator) - All results (Negative, Positive, Unknown)
    culture_done_count = compute_culture_result_recorded_denominator(df, facility_uids)
...
def compute_culture_result_recorded_trend_data(
    df, period_col="period_display", facility_uids=None, tei_df=None
):
    """
    Compute Culture Result Recorded trend data by period

    Returns:
        DataFrame with columns: period_display, culture_done_count, culture_result_count, culture_result_rate
    """
    if df is None or df.empty:
        return pd.DataFrame()
    ...
    for period in periods:
        ...
        period_kpi = compute_culture_result_recorded_kpi(
            period_df, facility_uids, tei_df
        )

        trend_data.append(
            {
                period_col: period,
                "culture_done_count": period_kpi["culture_done_count"],
                "culture_result_count": period_kpi["culture_result_count"],
                "culture_result_rate": period_kpi["culture_result_rate"],
            }
        )
...
# ---------------- CHART FUNCTIONS ----------------
def render_culture_result_recorded_trend_chart(
    df,
    ...
    # Compute trend data
    trend_df = compute_culture_result_recorded_trend_data(
    ...
    fig.update_traces(
        line=dict(width=3, color="#2ca02c"),  # Green color for result recorded
        marker=dict(size=8, color="#2ca02c"),
        hovertemplate="<b>%{x}</b><br>Result Recorded Rate: %{y:.1f}%<br>Result Recorded: %{customdata[0]}/%{customdata[1]}<extra></extra>",
        customdata=trend_df[["culture_result_count", "culture_done_count"]].values,
    )
    ...
    fig.update_layout(
        ...
        xaxis_title="Period",
        yaxis_title="Blood Culture Result Recorded Rate (%)",
        xaxis=dict(
            type="category",
            tickangle=-45,
            showgrid=True,
        ...
        yaxis=dict(
            range=[0, 105] if trend_df["culture_result_rate"].max() > 0 else [0, 100],
            showgrid=True,
            gridcolor="rgba(128,128,128,0.2)",
            zeroline=True,
    ...
    if len(trend_df) > 1:
        last_value = trend_df["culture_result_rate"].iloc[-1]
        prev_value = trend_df["culture_result_rate"].iloc[-2]
        trend_symbol = (
            "▲"
    ...
    # Show summary table
    st.subheader("📋 Blood Culture Result Recorded Trend by Period")

    if not trend_df.empty:
        # Calculate overall totals
        overall_data = compute_culture_result_recorded_kpi(df, facility_uids, tei_df)
        ...
        table_data = table_data.rename(
            columns={
                period_col: "Period",
                "culture_result_count": "Result Recorded (Negative/Positive)",
                "culture_done_count": "Total Culture Done (All Results)",
                "culture_result_rate": "Result Recorded Rate (%)",
            }
        )
        ...
        overall_row = {
            "Period": "Overall",
            "Result Recorded (Negative/Positive)": overall_data["culture_result_count"],
            "Total Culture Done (All Results)": overall_data["culture_done_count"],
            "Result Recorded Rate (%)": overall_data["culture_result_rate"],
        }
        ...
        column_order = [
            "Period",
            "Result Recorded (Negative/Positive)",
            "Total Culture Done (All Results)",
        ...
        styled_table = (
            table_data.style.format(
                {
                    "Result Recorded (Negative/Positive)": "{:,.0f}",
                    "Total Culture Done (All Results)": "{:,.0f}",
                    "Result Recorded Rate (%)": "{:.1f}%",
                }
            )
        ...
        st.download_button(
            label="Download CSV",
            data=csv,
            file_name="culture_result_recorded_trend.csv",
            mime="text/csv",
        )
    else:
...
def render_culture_result_recorded_facility_comparison_chart(
    df,
    period_col="period_display",
    ...
    """
    Render facility comparison chart for Culture Result Recorded KPI
    """
    if text_color is None:
        text_color = auto_text_color(bg_color)
    ...
    for period_display in all_periods:
        ...
        for facility_uid, facility_name in facility_uid_to_name.items():
            ...
            if not facility_period_df.empty:
                facility_kpi = compute_culture_result_recorded_kpi(
                ...
                time_series_data.append(
                    {
                        ...
                        "value": facility_kpi["culture_result_rate"],
                        "culture_result_count": facility_kpi["culture_result_count"],
                        "culture_done_count": facility_kpi["culture_done_count"],
                    }
                )
    ...
    fig.update_traces(
        line=dict(width=3),
        marker=dict(size=7),
        hovertemplate="<b>%{x}</b><br>%{data.name}: %{y:.1f}%<br>Result Recorded: %{customdata[0]}/%{customdata[1]}<extra></extra>",
        customdata=time_series_df[
            ["culture_result_count", "culture_done_count"]
        ].values,
    )
    ...
    for facility_uid, facility_name in facility_uid_to_name.items():
        ...
        if not facility_df.empty:
            culture_data = compute_culture_result_recorded_kpi(
            ...
            table_data.append(
                {
                    ...
                    "Result Recorded (Negative/Positive)": culture_data[
                        "culture_result_count"
                    ],
                    "Total Culture Done (All Results)": culture_data[
                        "culture_done_count"
                    ],
                    "Result Recorded Rate (%)": culture_data["culture_result_rate"],
                }
            )
    ...
    table_df = table_df.sort_values("Result Recorded Rate (%)", ascending=False)

    # Calculate overall totals
    overall_data = compute_culture_result_recorded_kpi(
        filtered_df, facility_uids, tei_df
    )
    ...
    st.download_button(
        label="Download CSV",
        data=csv,
        file_name="culture_result_recorded_facility_comparison.csv",
        mime="text/csv",
    )


def render_culture_result_recorded_region_comparison_chart(
    df,
    period_col="period_display",
    title="Blood Culture Result Recorded - Region Comparison",
    bg_color="#FFFFFF",
    text_color=None,
    region_names=None,
    region_mapping=None,
    ...
    """
    Render region comparison chart for Culture Result Recorded KPI
    """
    if text_color is None:
        text_color = auto_text_color(bg_color)
        ...
        for region_name in region_names:
            ...
            if region_facility_uids:
                ...
                if not region_period_df.empty:
                    region_kpi = compute_culture_result_recorded_kpi(
                    ...
                    time_series_data.append(
                        {
                            "period_display": period_display,
                            "Region": region_name,
                            "value": region_kpi["culture_result_rate"],
                            "culture_result_count": region_kpi["culture_result_count"],
                            "culture_done_count": region_kpi["culture_done_count"],
                        }
                    )
    ...
    st.download_button(
        label="Download CSV",
        data=csv,
        file_name="culture_result_recorded_region_comparison.csv",
        mime="text/csv",
    )


# ---------------- COMPREHENSIVE SUMMARY ----------------
def render_culture_result_recorded_comprehensive_summary(
    df,
    ...
    # Compute KPI
    culture_data = compute_culture_result_recorded_kpi(df, facility_uids, tei_df)
    ...
    if not culture_events.empty:
        ...
        result_data.append(
            {
                "Result": "TOTAL Recorded (Negative/Positive)",
                "Count": culture_counts.get("1", 0) + culture_counts.get("2", 0),
                "Percentage": (
                    (
                        (culture_counts.get("1", 0) + culture_counts.get("2", 0))
                        / len(culture_events)
                        * 100
                    )
                    if len(culture_events) > 0
...
__all__ = [
    ...
    "compute_culture_result_recorded_trend_data",
