...
def render_missing_cod_trend_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Condition of Discharge Documentation Rate Trend",
    bg_color="#FFFFFF",
    text_color=None,
    facility_names=None,
    ...
    chart_type = st.radio(
        ...
        index=0,
        horizontal=True,
        key=f"chart_type_missing_cod_{str(facility_uids)}",
    ).lower()
    ...
    if "numerator" in summary_df.columns and "denominator" in summary_df.columns:
        ...
        overall_row = pd.DataFrame(
            {
                ...
                "Missing CoD Rate (%)": [overall_value],
            }
        )
        ...
        summary_df = summary_df.rename(columns={value_col: "Missing CoD Rate (%)"})
        summary_table = summary_df.copy()

        overall_value = (
            summary_table["Missing CoD Rate (%)"].mean()
            if not summary_table.empty
            else 0
        )
        overall_row = pd.DataFrame(
            {x_axis_col: ["Overall"], "Missing CoD Rate (%)": [overall_value]}
        )
    ...
    st.download_button(
        ...
        data=csv,
        file_name="missing_cod_rate_trend_data.csv",
        mime="text/csv",
        help="Download the exact data shown in the chart",
    )


def render_missing_cod_facility_comparison_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Condition of Discharge Rate - Facility Comparison",
    bg_color="#FFFFFF",
    text_color=None,
    facility_names=None,
    facility_uids=None,
...
def render_missing_cod_region_comparison_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Condition of Discharge Rate - Region Comparison",
    bg_color="#FFFFFF",
    text_color=None,
    region_names=None,
    region_mapping=None,
