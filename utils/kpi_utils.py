...
def render_gauge_chart(
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                "steps": [
                    {
                        "range": [0, min_val + (max_val - min_val) * 0.33],
                        "color": steps_colors[0],
                    },
                    {
                        "range": [
    ...
    fig.update_layout(paper_bgcolor=bg_color, font_color=text_color, height=400)
...
def compute_fp_acceptance_count(df, facility_uids=None):
    ...
    # Count occurrences (not unique patients)
    fp_count = len(
        actual_events_df[
...
def compute_maternal_death_count(df, facility_uids=None):
    ...
    # Maternal deaths - count occurrences
    deaths_count = len(
        dfx[
...
def render_facility_comparison_chart(
    ...
    if len(other_cols) >= 2:
        ...
        if "IPPCAR" in title:
            ...
            overall_value = (
                facility_table_df["KPI Value"].mean()
        ...
        overall_value = (
            facility_table_df["KPI Value"].mean() if not facility_table_df.empty else 0
        )
...
def render_region_comparison_chart(
    ...
    other_cols = [
        c for c in region_table_df.columns if c not in ("Region Name", "KPI Value")
    ]
    if len(other_cols) >= 2:
        num_col, den_col = other_cols[0], other_cols[1]
        ...
        if "IPPCAR" in title:
            ...
            overall_value = (
                region_table_df["KPI Value"].mean() if not region_table_df.empty else 0
            )
    else:
        overall_numerator = 0
        overall_denominator = 0
    ...
    styled_table = (
        region_table_df.style.format(
            {
                region_table_df.columns[2]: (
                    "{:,.0f}" if len(region_table_df.columns) > 2 else "{:,.0f}"
                ),
