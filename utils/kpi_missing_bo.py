...
def render_missing_bo_trend_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Birth Outcome Documentation Rate Trend",
    bg_color="#FFFFFF",
    text_color=None,
    facility_names=None,
    ...
    """Render trend chart for Missing Birth Outcome - EXACT SAME AS ASSISTED"""
    ...
    chart_type = st.radio(
        ...
        index=0,
        horizontal=True,
        key=f"chart_type_missing_bo_{str(facility_uids)}",
    ).lower()
    ...
    if "numerator" in summary_df.columns and "denominator" in summary_df.columns:
        ...
        summary_df = summary_df.rename(columns={value_col: "Missing BO Rate (%)"})
        summary_table = summary_df.copy()

        overall_value = (
            summary_table["Missing BO Rate (%)"].mean()
            if not summary_table.empty
            else 0
        )
        overall_row = pd.DataFrame(
            {x_axis_col: ["Overall"], "Missing BO Rate (%)": [overall_value]}
        )
    ...
    st.download_button(
        ...
        data=csv,
        file_name="missing_bo_rate_trend_data.csv",
        mime="text/csv",
        help="Download the exact data shown in the chart",
    )


def render_missing_bo_facility_comparison_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Birth Outcome Rate - Facility Comparison",
    bg_color="#FFFFFF",
    text_color=None,
    facility_names=None,
    facility_uids=None,
...
def render_missing_bo_region_comparison_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Birth Outcome Rate - Region Comparison",
    bg_color="#FFFFFF",
    text_color=None,
    region_names=None,
    region_mapping=None,
