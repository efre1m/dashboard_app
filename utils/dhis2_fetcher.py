...
# DATA_ELEMENT_NAMES: MATERNAL FROM FIRST CODE, NEWBORN FROM SECOND CODE
DATA_ELEMENT_NAMES = {
    # Maternal Health (FROM FIRST CODE)
...
class CSVIntegration:
    ...
    def get_facility_region_mapping(
        ...
        # Map each facility to its region (vectorized)
        facility_names = df["orgUnit_name"].dropna()
        direct_match = facility_names.map(facility_to_region_map)
        df.loc[direct_match.index, "csv_region"] = direct_match

        # Partial matching for unmatched facilities
        unmatched_mask = df["csv_region"].eq("") & df["orgUnit_name"].notna()
        if unmatched_mask.any():
            unmatched_names = df.loc[unmatched_mask, "orgUnit_name"]
            for facility_key, region_name in facility_to_region_map.items():
                matched = unmatched_names.str.contains(facility_key, na=False) | unmatched_names.str.contains(facility_key, na=False)
                if matched.any():
                    df.loc[matched[matched].index, "csv_region"] = region_name
                    unmatched_names = df.loc[df["csv_region"].eq("") & df["orgUnit_name"].notna(), "orgUnit_name"]
    ...
    def transform_events_to_patient_level(
        ...
        FOR MATERNAL: Uses versioned columns for repeatable stages (FROM FIRST CODE)
        FOR NEWBORN: Uses the newer logic with repeatable Nurse followup Sheet (FROM SECOND CODE)
        """
        if events_df.empty:
        ...
        for _, stage_row in program_stages.iterrows():
            ...
            for tei_id, group in stage_groups:
                ...
                if is_maternal:
                    # ========== MATERNAL PROGRAM (FROM FIRST CODE) ==========
                    ...
                    if not is_repeatable:
                        ...
                        # REPEATABLE STAGE (Nurse followup Sheet): Create versioned columns
                        # FIRST: Group events by their event ID
                        event_groups = group.groupby("event")
                        ...
                        for event_id, event_group in event_groups:
                            ...
                            if not event_group.empty:
                                ...
                                for idx, row in event_group.iterrows():
                                    ...
                                    # Clean data element name
                                    clean_element_name = (
                                        CSVIntegration.clean_column_name(
                                            data_element_name
                                        )
                                    )
    ...
    def clean_transformed_dataframe(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Initialize event_date_filled_count to 0 (FIXED)
        ...
        try:
            ...
            for i, (region_uid, region_name) in enumerate(regions.items(), 1):
                ...
                tei_data = self.fetcher.fetch_program_data(
                    program_uid, region_uid, "DESCENDANTS", 5000
                )
    ...
    def process_selected_facilities_program(self, program_uid: str, program_name: str) -> bool:
        ...
        try:
            ...
            for i, facility in enumerate(self.selected_facilities, 1):
                ...
                tei_data = self.fetcher.fetch_program_data(
                    program_uid, facility_uid, "SELECTED", 5000
                )
                tei_count = len(tei_data.get("trackedEntityInstances", []))
                total_teis_fetched += tei_count
...
def main():
    """Main entry point - runs automated pipeline via CLI"""
    import sys
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Facility selection: 'all', comma-separated names/UIDs, or omit to prompt.",
    )
    parser.add_argument(
        "--merge-mode",
    ...
    if sys.argv.count("--facilities") > 0 and args.facilities is None:
        facility_selection = ""
