...
def compute_uterotonic_kpi(df, facility_uids=None):
    ...
    Returns:
        Dictionary with uterotonic metrics
    """
    if df is None or df.empty:
        return {
            "uterotonic_rate": 0.0,
...
def render_uterotonic_trend_chart(
    ...
    text_color=None,
    facility_names=None,
    numerator_name="uterotonic_count",
    ...
    # UPDATED: Chart options - removed regular Bar Chart, only Line and Stacked Percentage Bar
    ...
    if chart_type == "line chart":
        ...
        # UPDATED: True stacked percentage bar chart implementation
        if all(
            col in df.columns
            for col in ["ergometrine_count", "oxytocin_count", "misoprostol_count"]
        ):
            # Calculate percentages for each type relative to total deliveries
            stacked_df = df.copy()
            stacked_df["Ergometrine %"] = (
                stacked_df["ergometrine_count"] / stacked_df["total_deliveries"] * 100
            ).fillna(0)
            stacked_df["Oxytocin %"] = (
                stacked_df["oxytocin_count"] / stacked_df["total_deliveries"] * 100
            ).fillna(0)
            stacked_df["Misoprostol %"] = (
                stacked_df["misoprostol_count"] / stacked_df["total_deliveries"] * 100
            ).fillna(0)

            # Calculate the sum of all percentages for each period
            stacked_df["Total %"] = (
                stacked_df["Ergometrine %"]
                + stacked_df["Oxytocin %"]
                + stacked_df["Misoprostol %"]
            ...
            # Add traces for each uterotonic type
            fig.add_trace(
                go.Bar(
                    name="Ergometrine",
                    x=stacked_df[period_col],
                    y=stacked_df["Ergometrine %"],
                    marker_color="#1f77b4",
                    hovertemplate="<b>%{x}</b><br>Ergometrine: %{y:.1f}%<br>Count: %{customdata[0]}<br>Total Deliveries: %{customdata[1]}<extra></extra>",
                    customdata=np.column_stack(
                        (
                            stacked_df["ergometrine_count"],
                            stacked_df["total_deliveries"],
                        )
                    ),
                )
            )
            ...
            fig.add_trace(
                go.Bar(
                    name="Oxytocin",
                    x=stacked_df[period_col],
                    y=stacked_df["Oxytocin %"],
                    marker_color="#ff7f0e",
                    hovertemplate="<b>%{x}</b><br>Oxytocin: %{y:.1f}%<br>Count: %{customdata[0]}<br>Total Deliveries: %{customdata[1]}<extra></extra>",
                    customdata=np.column_stack(
                        (stacked_df["oxytocin_count"], stacked_df["total_deliveries"])
                    ),
                )
            )
            ...
            fig.add_trace(
                go.Bar(
                    name="Misoprostol",
                    x=stacked_df[period_col],
                    y=stacked_df["Misoprostol %"],
                    marker_color="#2ca02c",
                    hovertemplate="<b>%{x}</b><br>Misoprostol: %{y:.1f}%<br>Count: %{customdata[0]}<br>Total Deliveries: %{customdata[1]}<extra></extra>",
                    customdata=np.column_stack(
                        (
                            stacked_df["misoprostol_count"],
                            stacked_df["total_deliveries"],
                        )
                    ),
                )
            ...
            fig.update_layout(
                barmode="stack",
                title="Uterotonic Type Distribution Over Time",
                height=400,
                yaxis_title="Percentage of Women Receiving Uterotonic",
                xaxis_title="Period",
            )
        else:
    ...
    # Update traces for line/bar charts
    if chart_type == "line chart":
        fig.update_traces(
            line=dict(width=3),
            marker=dict(size=7),
        ...
        fig.update_layout(
            yaxis=dict(
                title="Percentage of Women Receiving Uterotonic",
                rangemode="tozero",
                showgrid=True,
