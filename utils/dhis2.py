...
def fetch_patient_data(program_uid: str, ou_uid: str) -> List[Dict[str, Any]]:
    """
    Fetch paginated TEIs for a program + orgUnit.
    """
    url = f"{settings.DHIS2_BASE_URL}/api/trackedEntityInstances.json"
    all_patients: List[Dict[str, Any]] = []
    page = 1
    logging.info(f"Fetching hierarchical patient data for OU: {ou_uid}")
...
def fetch_dhis2_data_for_ous(program_uid: str, ou_uids: List[str]) -> Dict[str, Any]:
    """
    Fetch all TEIs for all OUs in program, and enrich events with dataElement + programStage names.
    """
    all_patients: List[Dict[str, Any]] = []
    logging.info(f"Fetching hierarchical DHIS2 data for {len(ou_uids)} OUs")

    for ou_uid in ou_uids:
        patients = fetch_patient_data(program_uid, ou_uid)
