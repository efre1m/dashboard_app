"""
KPI: Missing Mode of Delivery Documentation
Measures: Percentage of deliveries where BOTH mode of delivery AND instrumental delivery are not documented
Formula: (Deliveries with missing documentation) ÷ (Total deliveries with enrollment dates) × 100
"""
...
# ---------------- Caching Setup ----------------
if "missing_md_cache" not in st.session_state:
    st.session_state.missing_md_cache = {}


def get_missing_md_cache_key(df, facility_uids=None, computation_type=""):
    """Generate a unique cache key for Missing MD computations"""
    key_data = {
...
def clear_missing_md_cache():
    """Clear the Missing MD cache"""
    st.session_state.missing_md_cache = {}


# ---------------- KPI Constants ----------------
MODE_OF_DELIVERY_COL = "mode_of_delivery_maternal_delivery_summary"
...
def compute_missing_md_count(df, facility_uids=None):
    """
    Count deliveries with missing mode of delivery documentation
    SIMPLE: Check if BOTH mode_of_delivery_maternal_delivery_summary AND instrumental_delivery_form are empty/N/A
    """
    cache_key = get_missing_md_cache_key(df, facility_uids, "missing_count")

    if cache_key in st.session_state.missing_md_cache:
        return st.session_state.missing_md_cache[cache_key]
    ...
    if df is None or df.empty:
        ...
        date_column = get_relevant_date_column_for_kpi(
            "Missing Mode of Delivery Documentation"
        )
        ...
        if filtered_df.empty:
            ...
            # SIMPLE CHECK: Both columns must be empty/N/A
            # Convert to string for comparison
            mode_vals = (
                filtered_df[MODE_OF_DELIVERY_COL].astype(str).str.strip()
                if MODE_OF_DELIVERY_COL in filtered_df.columns
                else pd.Series([""] * len(filtered_df))
            )
            inst_vals = (
                filtered_df[INSTRUMENTAL_DELIVERY_COL].astype(str).str.strip()
                if INSTRUMENTAL_DELIVERY_COL in filtered_df.columns
                else pd.Series([""] * len(filtered_df))
            )

            # Check if both are empty/N/A
            mode_empty = (
                mode_vals.isin(EMPTY_INDICATORS)
                | (mode_vals == "")
                | (mode_vals.str.upper() == "N/A")
            )
            inst_empty = (
                inst_vals.isin(EMPTY_INDICATORS)
                | (inst_vals == "")
                | (inst_vals.str.upper() == "N/A")
            )

            # Missing if BOTH are empty
            missing_mask = mode_empty & inst_empty
            result = int(missing_mask.sum())

    st.session_state.missing_md_cache[cache_key] = result
    return result


def compute_missing_md_rate(df, facility_uids=None):
    """
    Compute Missing Mode of Delivery Rate
    Returns: (rate, missing_cases, total_deliveries)
    """
    cache_key = get_missing_md_cache_key(df, facility_uids, "missing_rate")
    ...
    if df is None or df.empty:
        ...
        # Count missing cases
        missing_cases = compute_missing_md_count(df, facility_uids)
...
def compute_missing_md_kpi(df, facility_uids=None):
    """
    Compute Missing Mode of Delivery KPI data
    This is the function your dashboard will call
    """
    rate, missing_cases, total_deliveries = compute_missing_md_rate(df, facility_uids)

    return {
        "missing_md_rate": float(rate),
        "missing_md_cases": int(missing_cases),
        "total_deliveries": int(total_deliveries),
    }


def get_numerator_denominator_for_missing_md(
    df, facility_uids=None, date_range_filters=None
):
    """
    Get numerator and denominator for Missing MD KPI WITH DATE RANGE FILTERING
    ...
    # Get the SPECIFIC date column for Missing MD (delivery summary)
    date_column = get_relevant_date_column_for_kpi(
        "Missing Mode of Delivery Documentation"
    )
    ...
    # CRITICAL: Use the SAME logic as compute_missing_md_count
    # Check if BOTH columns are empty/N/A
    mode_vals = (
        filtered_df[MODE_OF_DELIVERY_COL].astype(str).str.strip()
        if MODE_OF_DELIVERY_COL in filtered_df.columns
        else pd.Series([""] * len(filtered_df))
...
def render_missing_md_trend_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Mode of Delivery Documentation Rate Trend",
    bg_color="#FFFFFF",
    text_color=None,
    facility_names=None,
    ...
    """Render trend chart for Missing MD - EXACT SAME AS ASSISTED"""
    ...
    chart_type = st.radio(
        ...
        index=0,
        horizontal=True,
        key=f"chart_type_missing_md_{str(facility_uids)}",
    ).lower()
    ...
    if "numerator" in summary_df.columns and "denominator" in summary_df.columns:
        ...
        summary_df = summary_df.rename(columns={value_col: "Missing MD Rate (%)"})
        summary_table = summary_df.copy()

        overall_value = (
            summary_table["Missing MD Rate (%)"].mean()
            if not summary_table.empty
            else 0
        )
        overall_row = pd.DataFrame(
            {x_axis_col: ["Overall"], "Missing MD Rate (%)": [overall_value]}
        )
    ...
    st.download_button(
        ...
        data=csv,
        file_name="missing_md_rate_trend_data.csv",
        mime="text/csv",
        help="Download the exact data shown in the chart",
    )


def render_missing_md_facility_comparison_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Mode of Delivery Rate - Facility Comparison",
    bg_color="#FFFFFF",
    text_color=None,
    facility_names=None,
    facility_uids=None,
...
def render_missing_md_region_comparison_chart(
    df,
    period_col="period_display",
    value_col="value",
    title="Missing Mode of Delivery Rate - Region Comparison",
    bg_color="#FFFFFF",
    text_color=None,
    region_names=None,
    region_mapping=None,
...
def prepare_data_for_missing_md_trend(
    df, kpi_name, facility_uids=None, date_range_filters=None
):
    """
    Prepare patient-level data for Missing MD trend chart
    ...
    # Get the SPECIFIC date column for Missing MD (enrollment_date)
...
# ---------------- Main Entry Point ----------------
def get_missing_md_kpi_data(df, facility_uids=None):
    """
    Main function to get Missing MD KPI data for dashboard
    """
